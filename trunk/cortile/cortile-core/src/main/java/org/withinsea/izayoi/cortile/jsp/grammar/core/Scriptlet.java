 ...
 * Time: 23:02:01
 */
public class Scriptlet implements CommentGrammar {

    @Override
    public boolean acceptComment(Comment comment) {
        return GrammarUtils.Accept.byTextPrefix(comment, "%");
    }
    ...
    public void processComment(DOMCompiler compiler, Compilr.Result result, Comment comment) throws CortileException {
        try {
            DOMUtils.replace(comment, "<%" + comment.getText().substring("%".length()).replaceAll("%$", "") + "%>");
        } catch (Exception e) {
