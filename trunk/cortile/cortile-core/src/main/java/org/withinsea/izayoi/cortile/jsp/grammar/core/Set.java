...
public class Set implements AttrGrammar {
    ...
    public boolean acceptAttr(Element elem, Attribute attr) {
        return GrammarUtils.Accept.byNamePrefix(attr, "set.");
    }

    @Override
    public void processAttr(DOMCompiler compiler, Compilr.Result result, Element elem, Attribute attr) throws CortileException {
        String subname = attr.getName().substring("set".length());
        try {
            if (subname.equals("")) {
                DOMUtils.insertBefore("<% " + elInterpreter.compileEL(attr.getValue()) + "; %>", elem);
            } else if (subname.startsWith(".")) {
                DOMUtils.insertBefore("<% " + elInterpreter.compileEL(subname.substring(1) + "=" + attr.getValue() + ";") + "; %>", elem);
            }
        } catch (Exception e) {
