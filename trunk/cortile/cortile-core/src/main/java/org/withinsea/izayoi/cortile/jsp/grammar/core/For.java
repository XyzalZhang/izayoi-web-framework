 ...
 * Time: 22:14:05
 */
public class For implements AttrGrammar {

    protected ELInterpreter elInterpreter;

    @Override
    public boolean acceptAttr(Element elem, Attribute attr) {
        return GrammarUtils.Accept.byNamePrefix(attr, "for.");
    }
    ...
    public void processAttr(DOMCompiler compiler, Compilr.Result result, Element elem, Attribute attr) throws CortileException {
        ...
        String iname = attr.getName().substring("for.".length());
        ...
        String helper = "varstack.put(\"" + iname + "\", " + iname + ");" +
                elInterpreter.compileEL("" +
                        iname + "_idx=(isdef " + iname + "_idx)?(" + iname + "_idx+1):0;" +
                        iname + "_odd=(isdef " + iname + "_odd)?(!" + iname + "_odd):false;" +
                        iname + "_even=(isdef " + iname + "_even)?(!" + iname + "_even):true;"
                ) + ";";

        try {
            DOMUtils.surround(elem, "<% for (Object " + iname + " : " + iter + ") { varstack.push(); " + helper + " %>", "<% varstack.pop(); } %>");
        } catch (Exception e) {
