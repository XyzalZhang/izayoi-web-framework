 ...
 * Time: 22:14:05
 */
public class Cycle implements AttrGrammar<HTMLCompiler> {
    ...
    public void processAttr(HTMLCompiler compiler, Compilr.Result result, Element elem, Attribute attr) throws CortileException {
        ...
        if (el.equals("") || el.indexOf("${") > 0 || el.matches(".*[^\\\\]}.*")) {
            throw new CortileException("\"" + attr.getValue() + "\" is not a valid EL interpret.");
        }
        ...
        if (attrname.startsWith("while")) {
            ...
            if (el.split("\\s*;\\s*").length == 3) {
                ...
                preScriptlet = preScriptlet + "for (Object " + i + ":" + "(Iterable)" + Cycle.class.getCanonicalName() +
