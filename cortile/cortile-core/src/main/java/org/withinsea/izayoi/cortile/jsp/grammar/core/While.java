 ...
 * Time: 23:09:11
 */
public class While implements AttrGrammar {

    protected ELInterpreter elInterpreter;

    @Override
    public boolean acceptAttr(Element elem, Attribute attr) {
        return GrammarUtils.Accept.byNamePrefix(attr, "while");
    }

    @Override
    public void processAttr(DOMCompiler compiler, Compilr.Result result, Element elem, Attribute attr) throws CortileException {
        try {
            DOMUtils.surround(elem, "<% while ((Boolean) " + elInterpreter.compileEL(attr.getValue()) + ") { %>", "<% } %>");
        } catch (Exception e) {
