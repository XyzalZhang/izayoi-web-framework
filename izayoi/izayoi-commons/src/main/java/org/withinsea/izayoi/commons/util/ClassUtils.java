...
public class ClassUtils {
    ...
    public static boolean isExtendsFrom(Class<?> sub, Class<?> supr) {
        while (sub != null) {
            if (sub == supr) {
                return true;
            }
            if (supr.isInterface()) {
                for (Class<?> intr : getAllInterfaces(sub)) {
                    if (intr == supr) {
                        return true;
                    }
                }
    ...
    public static Set<Class<?>> getAllInterfaces(Class<?> claz) {
        Set<Class<?>> intrs = new HashSet<Class<?>>();
        Deque<Class<?>> subs = new LinkedList<Class<?>>();
        subs.push(claz);
        do {
            Class<?> sub = subs.poll();
            for (Class<?> intr : sub.getInterfaces()) {
                if (!intrs.contains(intr)) {
                    subs.push(intr);
                    intrs.add(intr);
                }
            }
